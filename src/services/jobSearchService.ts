import { JobFilter, JobPosting, JobSearchResult, JobSearchResponse } from '../types/jobTypes.js';
...
export class JobSearchService {
  ...
  private filterJobs(jobs: JobPosting[], filter: JobFilter): JobPosting[] {
    return jobs.filter(job => {
      // Filter by job type (internship vs full-time)
      if (!filter.includeInternships && job.type === 'internship') return false;
      if (!filter.includeFullTime && job.type === 'full-time') return false;
      ...
      if (filter.requiredSkills.length > 0) {
        const hasRequiredSkills = filter.requiredSkills.every(skill =>
